...
data = pred - true_pred
fig.add_trace(
    go.Scatter(y=data[:, 0], name="pred - true",
               line=dict(
...
for i_d, d in enumerate(dimensions):
    ...
    # data = pca_pred[:, i_d] - pca_pred_true[:, i_d]
    data = meas.error_over_time(pca_pred[:, i_d][:, np.newaxis], pca_pred_true[:, i_d][:, np.newaxis],
...
fig.add_hline(0.4, row="all")

# SAVE
file_name = f"rpca_train_predict_time_series_difference.png"
