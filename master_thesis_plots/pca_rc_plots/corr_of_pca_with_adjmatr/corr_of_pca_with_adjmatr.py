...
for i_ens in range(n_ens):
    ...
    for i_train in range(n_train):
        ...
        if i_train == 0 and i_ens == 0:
            ...
            correlation_results = np.zeros((n_ens, n_train, n_components, r_dim))
        adj_matr = esn_obj.get_network()
        ...
        for i_r in range(r_dim):
            for i_comp in range(n_components):
                ...
                corr = np.correlate(adj_matr[:, i_r], components[i_comp, :])
                correlation_results[i_ens, i_train, i_comp, i_r] = corr
...
yaxis_title =  r"$\Large A_i, \text{Corr with adj.matr}$"
...
# SAVE
file_name = f"corr_of_pca_with_adjmatr.png"
