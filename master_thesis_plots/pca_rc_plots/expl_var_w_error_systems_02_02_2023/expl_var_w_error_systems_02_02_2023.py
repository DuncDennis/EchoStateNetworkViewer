...
sweep_dict = {
    "lorenz_beta": dict(
        sweep_name=r"\beta",
        sweep_values=[8/3, 0.2],
        sweep_value_names = ["chaotic input", "periodic input"],
        sweep_colors = [
            "#F37735",
            "#3DA4AB"
        ],
...
for i_sweep, sweep_value in enumerate(sweep_values):
    ...
    sys_obj = sims.Lorenz63(dt=0.1, beta=sweep_value)
...
for i_sweep, sweep_value in enumerate(sweep_values):
    ...
    hex_color = sweep_dict[sweep_key]["sweep_colors"][i_sweep]
    rgb_line = hex_to_rgba(hex_color, alpha=1)
    rgb_fill = hex_to_rgba(hex_color, alpha=0.35)
    # if sweep_dict[sweep_key]["full_val_range"] is None:
    ...
    fig.add_trace(
        go.Scatter(x=x, y=y, showlegend=True,
                   name=sweep_dict[sweep_key]["sweep_value_names"][i_sweep],
...
# SAVE
fig.write_image(f"expl_var_w_error_systems__{f_name}.png", scale=2.5)
